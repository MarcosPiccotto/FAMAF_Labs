...
int main(int argc, char *argv[])
{
    ...
    WeatherTable array;
    int tempMaxEveryYear[YEARS];
    unsigned int monthMostRainfall[YEARS][MONTHS];
    ...
    printf("\n La menor temperatura de toda la historia es: %.2f\n", minTempMin(array));
    maxTempEveryYear(array, tempMaxEveryYear);

    printf("los maximos:\n");
    for (unsigned int year = 0u; year < YEARS; ++year)
    {
        printf("%d\n", tempMaxEveryYear[year]);
    }

    printf("Los meses con mayor precipitacion:\n");
    monthOfMostRainfall(array, monthMostRainfall);
    for (unsigned int year = 0u; year < YEARS; ++year)
    {
        for (month_t month = january; month <= december; ++month)
        {
            printf("Del año%u, precipitaciones: %u\n", FST_YEAR + year, monthMostRainfall[year][month]);
        }
    }
    return (EXIT_SUCCESS);
}
