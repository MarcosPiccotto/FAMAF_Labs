...
queue queue_enqueue(queue q, queue_elem e)
{
    ...
    if (q->first == NULL)
    { // Si el primer nodo no existe lo creo
        q->first = q->last = new_node;
    }
    else
    { // Si ya existen nodos entonces
...
queue queue_disscard(queue q, unsigned int n)
{
    ...
    {
        // agarro el primero y el siguiente de ese
        struct s_node *current = q->first;
        struct s_node *nNext = current->next;
        // como se que n<size, reccorro la cola hasta que este en el anterior al que tengo que borrar
        while (n != 1u)
        {
            // sino estoy en el anterior avanzo sin problema
            current = current->next;
            nNext = nNext->next;
            n -= 1u;
        }
        // cuando estoy en la pos n-1 salteo el nodo en la pos n
        current->next = nNext->next;
        nNext = destroy_node(nNext);
        current = NULL;
    }
    q->size -= 1u;
    assert(invrep(q));
